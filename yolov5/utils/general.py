...
def check_suffix(file='yolov5s.pt', suffix=('.pt',), msg=''):
    ...
    if file and suffix:
        if isinstance(suffix, str):
            suffix = [suffix] # 如果suffix是字符串，则转为列表
        for f in file if isinstance(file, (list, tuple)) else [file]: # 将file转为列表
            # assert用法：assert 条件, "条件不成立时的报错信息"  即如果条件成立，代码继续运行
            assert Path(f).suffix.lower() in suffix, f"{msg}{f} acceptable suffix is {suffix}" # {msg}{f} 可接受的后缀是 {suffix}
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    ...
    path = Path(path)  # Path() 把字符串 变成 路径对象
    if path.exists() and not exist_ok:
        suffix = path.suffix             # 得到路径的扩展名,文件夹路径则为空
        path = path.with_suffix('')         # 去掉后缀，只保留文件名 / 文件夹名主体。
        dirs = glob.glob(f"{path}{sep}*")  # 找到所有名字相似的路径：
        matches = [re.search(rf"%s{sep}(\d+)" % path.stem, d) for d in dirs] # 用正则提取路径里的数字编号：
        i = [int(m.groups()[0]) for m in matches if m]  # 把提取到的编号变成整数列表，如 [2,3,4]
        n = max(i) + 1 if i else 2  # 如果有编号 → 最大编号 +1
        path = Path(f"{path}{sep}{n}{suffix}")  # 更新路径 原路径 + 编号 + 后缀
    dir = path if path.suffix == '' else path.parent  # 如果新路径后缀为空，则是文件夹路径，否则取它的父目录
    if not dir.exists() and mkdir: # 如果目录不存在，并且 mkdir=True
        dir.mkdir(parents=True, exist_ok=True)  # make directory
    return path
